...
Socket.prototype.connect = function(port, host, fn){
  ...
  var maxTimeout = self.get('retry max timeout');
  var sock;
  var connectEvent;
  ...
  sock.on('close', function(){
    ...
    if (self.retry >= self.get('max retry')) {
      debug('%s reconnect reach max times', self.type);
      return self.emit('close');
    }
    var retryTimeout = Math.round(Math.min(maxTimeout, self.get('retry timeout') * Math.pow(1.5, self.retry)));
    self.retry++;
    setTimeout(function(){
